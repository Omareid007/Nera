...
export function AnalyticsPage() {
  ...
  const chartData: CandlePoint[] = data ? data.candles.map((c, i) => ({
    ...
    sma20: data.sma20[i] ?? undefined,
    sma50: data.sma50[i] ?? undefined,
    ema12: data.ema12[i] ?? undefined,
    ema26: data.ema26[i] ?? undefined,
    bollingerUpper: data.bollingerUpper[i] ?? undefined,
    bollingerLower: data.bollingerLower[i] ?? undefined,
    bollingerMid: data.bollingerMid[i] ?? undefined,
    rsi: data.rsi14[i] ?? undefined,
    macd: data.macd[i] ?? undefined,
    macdSignal: data.macdSignal[i] ?? undefined,
    macdHist: data.macdHistogram[i] ?? undefined,
    volumeSma: data.volumeSma20[i] ?? undefined,
  })) : [];
